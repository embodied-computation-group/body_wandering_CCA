...
% only subjs in body-wandering analysis
SubIDs = readtable('/home/leah/Git/BodyWanderingCCA/data/data4correlations/EGGmetric_subIDs.csv')
SubIDs.x = extractAfter(string(SubIDs.x), strlength(SubIDs.x) -4);
...
%        % skip subjects not in body-wandering EGG analysis
%     if ~(ismember(files(n).name(1:4), SubIDs.x))
%         continue
%     end
...
MW = readtable('/home/leah/Git/BodyWanderingCCA/data/mindwand_movieratings.tsv', 'TreatAsEmpty',{'NA'}, 'Delimiter', '\t', 'FileType', 'text'); % load all psych scores 
MW_SubID = str2double(extractAfter(MW{:,1}, 5));
MW = table2array(MW(:,2:23)); 
MW = MW((find(ismember(MW_SubID(:,1), SubID))),:);
% body/cognitive/neg-wandering scores
Body_MW = mean(MW(:,15:22)');
Cog_MW = mean(MW(:,1:4)');
Neg_MW = MW(:,6)'- MW(:,5)';
% 
Goodsubs_psych = Body_MW(find(ismember(SubID, Good_subs)));
Badsubs_psych = Body_MW(find(ismember(SubID, Bad_subs)));
