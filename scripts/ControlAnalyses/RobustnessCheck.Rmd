...
# Psychophysiological Correlations - ITEMS - Robustness
```{r}
...
# Compute Spearman correlation for each group - MENTAL HEALTH
corr_results <- list()
pval_results <- list()
for (key in names(datasets)) {
  corr_matrix <- psych::corr.test(datasets[[key]], method = "spearman", adjust = "fdr")$r[1:22, 23:24]
  p_matrix <- psych::corr.test(datasets[[key]], method = "spearman", adjust = "fdr")$p[1:22, 23:24]
  corr_results[[key]] <- as.data.frame(t(corr_matrix))
  corr_results[[key]]$Vars <- rownames(corr_results[[key]])
  # Convert to dataframe and store p-values
  pval_results[[key]] <- as.data.frame(t(p_matrix))
...
# Identify significant correlations in orig full dataset
cor_pvals <- t(psych::corr.test(corrdata, method = "spearman", adjust = "fdr")$p[1:22, 23:24])
sig_mask <- cor_pvals < 0.05
...
# Print ICC result
print(paste("Intraclass Correlation Coefficient (ICC) using significant MENTALHEALTH correlations:", 
            round(icc_result$value, 3)))
...
# Compute Spearman correlation for each group - PHYSIOLOGICAL AROUSAL
corr_results <- list()
pval_results <- list()
for (key in names(datasets)) {
  corr_matrix <- psych::corr.test(datasets[[key]], method = "spearman", adjust = "fdr")$r[1:22, 25:26]
  p_matrix <- psych::corr.test(datasets[[key]], method = "spearman", adjust = "fdr")$p[1:22, 25:26]
  corr_results[[key]] <- as.data.frame(t(corr_matrix))
  corr_results[[key]]$Vars <- rownames(corr_results[[key]])
  # Convert to dataframe and store p-values
  pval_results[[key]] <- as.data.frame(t(p_matrix))
...
# Identify significant correlations in orig full dataset
cor_pvals <- t(psych::corr.test(corrdata, method = "spearman", adjust = "fdr")$p[1:22, 25:26])
sig_mask <- cor_pvals < 0.05
...
# Print ICC result
print(paste("Intraclass Correlation Coefficient (ICC) using significant AROUSAL correlations:", 
            round(icc_result$value, 3)))
